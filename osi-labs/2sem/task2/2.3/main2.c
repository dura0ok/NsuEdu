...
void *compare_length_thread(void *data)
{
    ...
    while (1) {
        ...
        if (pthread_spin_lock(&storage->sync) != 0) {
            perror("compare_length_thread: pthread_spin_lock(0) failed");
            break;
        }
        ...
        if (pthread_spin_lock(&curr1->sync) != 0) {
            perror("compare_length_thread: pthread_spin_lock(1) failed");
            pthread_spin_unlock(&storage->sync);
            break;
        }
        ...
        while (curr2 != NULL)
        {
            ...
            if(pthread_spin_lock(&curr2->sync) != 0)
            {
                perror("compare_length_thread: pthread_spin_lock(2) failed");
                break;
            }
...
void *swap_thread(void *data)
{
    ...
    while (1) {
        ...
        if (pthread_spin_lock(&storage->sync) != 0) {
            perror("swap_thread: pthread_spin_lock(0) failed");
            break;
        }
        ...
        if (pthread_spin_lock(&curr1->sync) != 0) {
            perror("swap_thread: pthread_spin_lock(1) failed");
            pthread_spin_unlock(&storage->sync);
            break;
        }
        ...
        if (pthread_spin_lock(&curr2->sync) != 0) {
            perror("swap_thread: pthread_spin_lock(2) failed");
            pthread_spin_unlock(&curr1->sync);
            pthread_spin_unlock(&storage->sync);
            break;
        }
        ...
        while (curr3 != NULL) {
            ...
            if(pthread_spin_lock(&curr3->sync) != 0)
            {
                perror("swap_thread: pthread_spin_lock(3) failed");
                break;
            }
...
void *count_monitor(void *arg) {
    ...
    while (1) {
        ...
        if (pthread_spin_lock(&storage->sync) != 0) {
            perror("count_monitor: pthread_spin_lock(0) failed");
            break;
        }
        ...
        if (pthread_spin_lock(&curr1->sync) != 0) {
            perror("count_monitor: pthread_spin_lock(1) failed");
            pthread_spin_unlock(&storage->sync);
            break;
        }
        ...
        while (curr2 != NULL)
        {
            ...
            if(pthread_spin_lock(&curr2->sync) != 0)
            {
                perror("count_monitor: pthread_spin_lock(2) failed");
                break;
            }
