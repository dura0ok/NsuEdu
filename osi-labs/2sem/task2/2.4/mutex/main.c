...
void *compare_length_thread(void *data)
{
    ...
    while (1) {
        ...
        if (custom_mutex_lock(&storage->sync) != 0) {
            perror("compare_length_thread: custom_mutex_lock(0) failed");
            break;
        }
        ...
        if (custom_mutex_lock(&curr1->sync) != 0) {
            perror("compare_length_thread: custom_mutex_lock(1) failed");
            custom_mutex_unlock(&storage->sync);
            break;
        }
        ...
        while (curr2 != NULL)
        {
            ...
            if(custom_mutex_lock(&curr2->sync) != 0)
            {
                perror("compare_length_thread: custom_mutex_lock(2) failed");
                break;
            }
...
void *swap_thread(void *data)
{
    ...
    while (1) {
        ...
        if (custom_mutex_lock(&storage->sync) != 0) {
            perror("swap_thread: custom_mutex_lock(0) failed");
            break;
        }
        ...
        if (custom_mutex_lock(&curr1->sync) != 0) {
            perror("swap_thread: custom_mutex_lock(1) failed");
            custom_mutex_unlock(&storage->sync);
            break;
        }
        ...
        if (custom_mutex_lock(&curr2->sync) != 0) {
            perror("swap_thread: custom_mutex_lock(2) failed");
            custom_mutex_unlock(&curr1->sync);
            custom_mutex_unlock(&storage->sync);
            break;
        }
        ...
        while (curr3 != NULL) {
            ...
            if(custom_mutex_lock(&curr3->sync) != 0)
            {
                perror("swap_thread: custom_mutex_lock(3) failed");
                break;
            }
...
void *count_monitor(void *arg) {
    ...
    while (1) {
        ...
        if (custom_mutex_lock(&storage->sync) != 0) {
            perror("count_monitor: custom_mutex_lock(0) failed");
            break;
        }
        ...
        if (custom_mutex_lock(&curr1->sync) != 0) {
            perror("count_monitor: custom_mutex_lock(1) failed");
            custom_mutex_unlock(&storage->sync);
            break;
        }
        ...
        while (curr2 != NULL)
        {
            ...
            if(custom_mutex_lock(&curr2->sync) != 0)
            {
                perror("count_monitor: custom_mutex_lock(2) failed");
                break;
            }
