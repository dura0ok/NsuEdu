...
void *compare_length_thread(void *data) {
    ...
    while (1) {
        ...
        Node *curr2, *tmp;
        while (1) {
            if (curr != NULL && pthread_rwlock_rdlock(&curr->rwlock) == 0) {
                if (curr->next != NULL && pthread_rwlock_rdlock(&curr->next->rwlock) == 0) {
...
_Noreturn void *swap_thread(void *data) {
    ...
    while (1) {
        ...
        while (1) {
            if (curr1 != NULL && pthread_rwlock_wrlock(&curr1->rwlock) == 0) {
                if (curr1->next != NULL && pthread_rwlock_wrlock(&curr1->next->rwlock) == 0) {
                    if (curr1->next->next != NULL && pthread_rwlock_wrlock(&curr1->next->next->rwlock) == 0) {
                        curr2 = curr1->next;
                        ...
                        curr2 = curr1->next;
                        pthread_rwlock_unlock(&tmp->rwlock);
                        pthread_rwlock_unlock(&curr1->rwlock);
                        pthread_rwlock_unlock(&curr2->rwlock);
                    }
                    else {
                        tmp = curr1;
                        curr1 = curr1->next;
